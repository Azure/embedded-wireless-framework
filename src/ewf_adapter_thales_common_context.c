 ...
 * @brief The EWF Common Thales adapter driver context management implementation
 ****************************************************************************/
...
ewf_result ewf_adapter_thales_common_context_activate(ewf_adapter* adapter_ptr, uint32_t context_id)
{
    ...
    if (ewf_result_failed(result = ewf_interface_send_commands(interface_ptr, "AT^SICA=1,", context_id_cstr, "\r", NULL))) return result;
...
ewf_result ewf_adapter_thales_common_context_deactivate(ewf_adapter* adapter_ptr, uint32_t context_id)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_interface* interface_ptr = adapter_ptr->interface_ptr;
    EWF_INTERFACE_VALIDATE_POINTER(interface_ptr);
    ...
    if (ewf_result_failed(result = ewf_interface_send_commands(interface_ptr, "AT^SICA=0,", context_id_cstr,"\r", NULL))) return result;
    if (ewf_result_failed(result = ewf_interface_verify_response(interface_ptr, "\r\nOK\r\n"))) return result;
...
ewf_result ewf_adapter_thales_common_context_configure(ewf_adapter* adapter_ptr, uint32_t context_id, uint32_t type, const char * const apn, const char * const username, const char * const password, uint32_t authentication)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_interface* interface_ptr = adapter_ptr->interface_ptr;
    EWF_INTERFACE_VALIDATE_POINTER(interface_ptr);
...
ewf_result ewf_adapter_thales_common_context_configure_dns(ewf_adapter* adapter_ptr, uint32_t context_id, const char * const primary_dns, const char * const secondary_dns)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_interface* interface_ptr = adapter_ptr->interface_ptr;
    EWF_INTERFACE_VALIDATE_POINTER(interface_ptr);
