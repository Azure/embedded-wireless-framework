...
ewf_result ewf_adapter_renesas_common_start(ewf_adapter* adapter_ptr)
{
    ...
    result = ewf_adapter_renesas_common_tokenizer_init(interface_ptr);
    ...
    /* Disable Registration URC */
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+CREG=0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    /* Disable EPS Registration URC */
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+CEREG=0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;
