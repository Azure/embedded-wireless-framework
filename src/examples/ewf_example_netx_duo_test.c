...
#include "ewf_example_netx_duo_test.h"
...
ewf_result ewf_example_netx_duo_test(ewf_adapter* adapter_ptr)
{
    ...
    result = ewf_adapter_get_ipv4_address(adapter_ptr, (uint32_t*)&ip_address);
    ...
    result = ewf_adapter_get_ipv4_netmask(adapter_ptr, (uint32_t*)&network_mask);
    if (ewf_result_failed(result))
    {
    ...
    result = ewf_adapter_get_ipv4_gateway(adapter_ptr, (uint32_t*)&gateway_address);
    if (ewf_result_failed(result))
    {
    ...
    result = ewf_adapter_get_ipv4_dns(adapter_ptr, (uint32_t*)&dns_address);
    if (ewf_result_failed(result))
    {
