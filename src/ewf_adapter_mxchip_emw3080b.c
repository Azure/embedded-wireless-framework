 ...
 * @brief The Embedded Wireless Framework MX-CHIP EMW3080B adapter API
 ****************************************************************************/
...
{
    ewf_adapter_mxchip_emw3080b_start,
    ewf_adapter_mxchip_emw3080b_stop,
};
...
ewf_result ewf_adapter_mxchip_emw3080b_start(ewf_adapter* adapter_ptr)
{
    ...
    if (ewf_result_failed(result = ewf_adapter_mxchip_common_start(adapter_ptr)))
    {
        return result;
    }
...
ewf_result ewf_adapter_mxchip_emw3080b_stop(ewf_adapter* adapter_ptr)
{
    ...
    if (ewf_result_failed(result = ewf_adapter_mxchip_common_stop(adapter_ptr)))
    {
        return result;
    }
...
ewf_result ewf_adapter_mxchip_emw3080b_urc_callback(ewf_interface* interface_ptr, uint8_t* buffer_ptr, uint32_t buffer_length)
{
    ...
    return ewf_adapter_mxchip_common_urc_callback(interface_ptr, buffer_ptr, buffer_length);
}
