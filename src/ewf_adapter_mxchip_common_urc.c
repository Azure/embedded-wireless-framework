...
ewf_result ewf_adapter_mxchip_common_urc_callback(ewf_interface* interface_ptr, uint8_t * buffer_ptr, uint32_t buffer_length)
{
    ...
    if (ewfl_str_equals_str((char*)buffer_ptr, "\r\n+WEVENT:AP_UP\r\n"))
    {
        implementation_ptr->status_ap = true;
        return EWF_RESULT_OK;
    }

    if (ewfl_str_equals_str((char*)buffer_ptr, "\r\n+WEVENT:AP_DOWN\r\n"))
    {
        implementation_ptr->status_ap = false;
        return EWF_RESULT_OK;
    }

    if (ewfl_str_equals_str((char*)buffer_ptr, "\r\n+WEVENT:STATION_UP\r\n"))
    {
        implementation_ptr->status_station = true;
        return EWF_RESULT_OK;
    }

    if (ewfl_str_equals_str((char*)buffer_ptr, "\r\n+WEVENT:STATION_DOWN\r\n"))
    {
        implementation_ptr->status_station = false;
        return EWF_RESULT_OK;
    }

#if EWF_ADAPTER_MXCHIP_COMMON_TCP_ENABLED || EWF_ADAPTER_MXCHIP_COMMON_UDP_ENABLED
    /* Call the internet URC callback */
    if (ewf_result_failed(result = ewf_adapter_mxchip_common_internet_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
