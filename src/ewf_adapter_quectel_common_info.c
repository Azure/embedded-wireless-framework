...
ewf_adapter_api_info ewf_adapter_quectel_common_api_info =
{
    ewf_adapter_quectel_common_info,
    ewf_adapter_quectel_common_get_ipv4_address,
    ewf_adapter_quectel_common_get_ipv4_netmask,
    ewf_adapter_quectel_common_get_ipv4_gateway,
    ewf_adapter_quectel_common_get_ipv4_dns,
    NULL,
    NULL,
    NULL,
};
...
ewf_result ewf_adapter_quectel_common_info(ewf_adapter* adapter_ptr)
{
    ...
    /* Show ICCID */
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QCCID\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;
    ...
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QCSQ\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    /* Get band */
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QNWINFO\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;
    ...
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"version\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"pdpcid\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"will\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"timeout\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"session\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"prefix\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"keepalive\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;

    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"ssl\",0\r"))) return result;
    if (ewf_result_failed(result = ewf_interface_drop_response(interface_ptr))) return result;
...
ewf_result ewf_adapter_quectel_common_get_ipv4_gateway(ewf_adapter* adapter_ptr, uint32_t * gateway_ptr)
{
    ...
    return ewf_adapter_quectel_common_get_ipv4_address(adapter_ptr, gateway_ptr);
}
