...
ewf_result ewf_adapter_thales_common_start(ewf_adapter* adapter_ptr)
{
    ...
    result = ewf_adapter_thales_common_tokenizer_init(interface_ptr);
    if (ewf_result_failed(result))
    {
...
#if EWF_ADAPTER_THALES_COMMON_FFS_ENABLED
    if (ewf_result_failed(result = ewf_adapter_thales_common_ffs_stop(adapter_ptr)))
    {
