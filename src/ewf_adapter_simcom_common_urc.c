...
#if EWF_ADAPTER_SIMCOM_COMMON_TLS_ENABLED
    if (ewf_result_failed(result = ewf_adapter_simcom_common_tls_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif

#if EWF_ADAPTER_SIMCOM_COMMON_TLS_BASIC_ENABLED
    if (ewf_result_failed(result = ewf_adapter_simcom_common_tls_basic_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif

#if EWF_ADAPTER_SIMCOM_COMMON_TCP_ENABLED || EWF_ADAPTER_SIMCOM_COMMON_UDP_ENABLED
    /* Call the internet URC callback */
    if (ewf_result_failed(result = ewf_adapter_simcom_common_internet_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif

#if EWF_ADAPTER_SIMCOM_COMMON_MQTT_ENABLED
    /* Call the MQTT URC callback */
    if (ewf_result_failed(result = ewf_adapter_simcom_common_mqtt_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif

#if EWF_ADAPTER_SIMCOM_COMMON_MQTT_BASIC_ENABLED
    /* Call the MQTT URC callback */
    if (ewf_result_failed(result = ewf_adapter_simcom_common_mqtt_basic_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif
