...
ewf_result ewf_adapter_quectel_common_mqtt_basic_urc_callback(ewf_interface* interface_ptr, uint8_t* buffer_ptr, uint32_t buffer_length)
{
    ...
    if (ewfl_str_starts_with((char*)buffer_ptr, urc_prefix_str))
    {
...

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                implementation_ptr->mqtt_basic_publish = true;
                /* Message published */
...

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                implementation_ptr->mqtt_basic_subscribe = true;
                /* Subscribed to topic */
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                /* Parse the topic and payload */
                {
                    ...
                    uint32_t payload_len = 0;
                    {
                        ...
                        q3 = ewfl_find_chars_with_terms(q2, ",", NULL);
                        if (q1 && q2)
                        {
                            topic = q1+1;
                            *q2 = 0;
                        ...
                        /* Skip "," after payload lenght in the URC */
                        q3++;
                        payload_len_str = q3;
                        payload_len = ewfl_str_to_unsigned(payload_len_str);

                        q3 = ewfl_find_chars_with_terms(q3, ",", NULL);

                        /* Skip the starting ",\" before the payload*/
                        payload = q3 + 2;
                        q3[payload_len + 2] = 0;
...
ewf_result ewf_adapter_quectel_common_mqtt_basic_connect(ewf_adapter* adapter_ptr, char const* server_str, uint32_t port, char const* clientid, char const* username, char const* password)
{
    ...
    /* Configure MQTT receive URC handling */
    if (ewf_result_failed(result = ewf_interface_send_command(interface_ptr, "AT+QMTCFG=\"recv/mode\",0,0,1\r"))) return result;
...
ewf_result ewf_adapter_quectel_common_mqtt_basic_subscribe(ewf_adapter* adapter_ptr, char const* topic_cstr)
{
    ...
    {
        if (implementation_ptr->mqtt_basic_subscribe) break;
        ewf_interface_poll(interface_ptr);
        ewf_platform_sleep(1);
    }
    implementation_ptr->mqtt_basic_subscribe = false;
...
ewf_result ewf_adapter_quectel_common_mqtt_basic_publish(ewf_adapter* adapter_ptr, const char* topic_cstr, const char* message_cstr)
{
    ...
    const char* message_length_cstr = ewfl_unsigned_to_str(message_length, message_length_str, sizeof(message_length_str));

    if (ewf_result_failed(result = ewf_interface_send_commands_wait_for_prompt(
        interface_ptr,
        "\r\n >",
        "AT+QMTPUB=0,0,0,0,\"", topic_cstr, "\",", message_length_cstr, "\r", NULL)))
            return result;
    ...
    for (unsigned i = 0; i < (2 * EWF_PLATFORM_TICKS_PER_SECOND); i++)
    {
        if (implementation_ptr->mqtt_basic_publish) break;
        ewf_interface_poll(interface_ptr);
        ewf_platform_sleep(1);
    }
    implementation_ptr->mqtt_basic_publish = false;
    return EWF_RESULT_OK;
}
