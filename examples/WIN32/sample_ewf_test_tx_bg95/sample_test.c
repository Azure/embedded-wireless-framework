...
void thread_sample_entry(ULONG param)
{
    ...
    EWF_ALLOCATOR_THREADX_STATIC_DECLARE(message_allocator_ptr, message_allocator, 32, 128);
    ...
    EWF_ADAPTER_QUECTEL_BG95_STATIC_DECLARE(adapter_ptr, quectel_bg95, message_allocator_ptr, data_allocator_ptr, interface_ptr);
    ...
    {
        EWF_LOG_ERROR("Failed to run the adapter test: return code 0x%08lx.", result);
        return;
    }
