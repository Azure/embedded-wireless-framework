...
/*-----------------------------------------------------------*/
static volatile uint8_t tcp_initialised = 0;

static void tcpip_init_complete(void* arg)
{
    tcp_initialised = 1;
}
...
int main( void )
{
    ...
    static StackType_t pppRxTaskStack[MAIN_STACK_SIZE * sizeof(StackType_t)];
    ...
    xTaskCreateStatic(pppos_rx_thread_task, "PPP RX task", MAIN_STACK_SIZE, NULL, EWF_PLATFORM_THREAD_PRIORITY_MEDIUM, pppRxTaskStack, &PPPRxTask);

    /* Initialize the LWIP stack */
    tcpip_init(tcpip_init_complete, NULL);
...
static void prvMainTask( void *pvParameters )
{
    ...
    EWF_ADAPTER_THALES_EXS82_W_STATIC_DECLARE(adapter_ptr, thales_exs82_w, message_allocator_ptr, NULL, interface_ptr);
    ...
    // Check if the modem is registered to the SIM card carrier network
    ...
    // Call the LWIP PPP test example
    if (ewf_result_failed(result = ewf_example_lwip_ppp_test(adapter_ptr)))
    {
