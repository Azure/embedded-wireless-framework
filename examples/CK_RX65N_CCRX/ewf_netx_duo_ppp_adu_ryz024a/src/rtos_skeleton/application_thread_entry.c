...
void user_ppp_invalid_packet_handler(NX_PACKET *packet_ptr)
{
...
ewf_netxduo_ppp_cfg ppp_cfg =
{
    ...
    .ppp_invalid_packet_callback = user_ppp_invalid_packet_handler,
    .ip_ptr = &ip_0,
    .ip_thread_stack_ptr = (uint8_t*)&sample_ip_stack[0],
    .ip_thread_stack_size = sizeof(sample_ip_stack),
    .pool_ptr = &pool_0
};
...
void application_thread_entry(ULONG entry_input)
{
	...
	EWF_ADAPTER_RENESAS_RYZ024A_STATIC_DECLARE(adapter_ptr, renesas_ryz024a, message_allocator_ptr, NULL, interface_ptr);    // Start the adapter
...
//    }
 ...
 //   nx_ip_gateway_address_set(&ip_0, g_gateway_address);
    ...
    if (status)
    {
        demo_printf("SNTP Time Sync failed.\r\n");
        demo_printf("Set Time to default value: SAMPLE_SYSTEM_TIME.");
        unix_time_base = SAMPLE_SYSTEM_TIME;
    }
    else
    {
        demo_printf("SNTP Time Sync successfully.\r\n");
    }
...
ULONG   dns_server_address[3];
    ...
    /* Get DNS address */
    status = nx_ppp_dns_address_get(&ppp_0, (ULONG*)&g_dns_address);
    if (status)
    {
        nx_dns_delete(&dns_0);
        return(status);
    ...
    status = nxd_dns_host_by_name_get(&dns_0, (UCHAR *)"www.microsoft.com", &server_address, NX_IP_PERIODIC_RATE, NX_IP_VERSION_V4);
